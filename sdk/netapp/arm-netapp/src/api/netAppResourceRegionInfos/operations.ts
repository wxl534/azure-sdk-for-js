...
export async function _listDeserialize(result: PathUncheckedResponse): Promise<_RegionInfosList> {
...
export async function _getDeserialize(result: PathUncheckedResponse): Promise<RegionInfoResource> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    const error = createRestError(result);
    if (result.body) {
