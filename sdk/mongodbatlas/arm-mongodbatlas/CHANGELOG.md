# Release History

## 1.1.0-beta.1 (2026-06-09)
Compared with version 1.0.0

### Features Added
  - Added operation group ClustersOperations
  - Added operation group ProjectsOperations
  - Added Interface Cluster
  - Added Interface ClusterProperties
  - Added Interface ClustersCreateOrUpdateOptionalParams
  - Added Interface ClustersDeleteOptionalParams
  - Added Interface ClustersGetOptionalParams
  - Added Interface ClustersListOptionalParams
  - Added Interface Project
  - Added Interface ProjectLimitStatus
  - Added Interface ProjectProperties
  - Added Interface ProjectsCreateOrUpdateOptionalParams
  - Added Interface ProjectsDeleteOptionalParams
  - Added Interface ProjectsGetOptionalParams
  - Added Interface ProjectsListClusterTierRegionsOptionalParams
  - Added Interface ProjectsListOptionalParams
  - Added Interface ProjectsTierLimitReachedOptionalParams
  - Added Interface ProxyResource
  - Added Interface RegionsByTierResponse
  - Added Interface TierLimitReachedResponse
  - Added Interface TierRegions
  - Interface AtlasClientOptionalParams has a new optional parameter cloudSetting
  - Added Type Alias AzureSupportedClouds
  - Added Type Alias ClusterTier
  - Added Enum AzureClouds
  - Added Enum KnownClusterTier
  - Enum KnownVersions has a new value V20241118Preview
  - Enum KnownVersions has a new value V20260301Preview

    
## 1.0.0 (2025-07-02)

### Features Added

This is the first stable version with the package of @azure/arm-mongodbatlas
