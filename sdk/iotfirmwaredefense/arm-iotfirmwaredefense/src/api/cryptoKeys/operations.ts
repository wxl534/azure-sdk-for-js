...
export function _listByFirmwareSend(
  ...
  firmwareId: string,
  options: CryptoKeysListByFirmwareOptionalParams = { requestOptions: {} },
): StreamableMethod {
  const path = expandUrlTemplate(
    "/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.IoTFirmwareDefense/workspaces/{workspaceName}/firmwares/{firmwareId}/cryptoKeys{?api%2Dversion}",
    {
...
export async function _listByFirmwareDeserialize(
...
): Promise<_CryptoKeyResourceListResult> {
  ...
  return _cryptoKeyResourceListResultDeserializer(result.body);
}
